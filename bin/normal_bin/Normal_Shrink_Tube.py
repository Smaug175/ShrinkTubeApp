...
from bin.normal_bin.model.Normal.CuttingClipMold import DC0124_AD03, DC0124_AD03_, DC0121_AD03, DC0121_AD04, DC0125_AD07
from bin.normal_bin.model.Normal.FormingMandrel import DC0121_SS01, DC0124_SS01, DC0125_SS01
from bin.normal_bin.model.Normal.FormingMold import DIEO
from bin.normal_bin.model.Normal.FormingUnloadingMold import DC0124_AD02, DC0121_AD02, DC0125_AD06_F
from bin.normal_bin.model.Normal.ShrinkTubeMandrel import ADBT
from bin.normal_bin.model.Normal.ShrinkTubeMold import ADIE
from bin.normal_bin.model.Normal.ShrinkTubeUnloadingMold import DC0124_AD01, DC0121_AD01, DC0125_AD06_S
from bin.normal_bin.model.Normal_Tube_Model import TubeModelClass
...
class ShrinkTubeClass:
    ...
    def __init__(self, file_path):
        """初始化ShrinkTubeClass"""
        # 导入模具的参数描述和计算方法
        with open('bin/normal_bin/model/Normal/Parameter_Description.json', 'r', encoding='utf-8') as file:
            self.Parameter_Description = json.load(file)
        with open('bin/normal_bin/model/Normal/Parameter_Calculate_Method.json', 'r', encoding='utf-8') as file:
    ...
    def calculate(self, user_name: str, Normal_Add: bool, mold_list: list, machine_type: str) -> dict:
        ...
        if machine_type == 'DC0124':
            ...
            for mold_name in self.Mold_Object:
                ...
                self.external_params['图号'] = self.get_numbers(mold_name, machine_type)
                if mold_name == '裁剪夹模':
            ...
            if not Normal_Add and len(Tx) > 2 and '裁剪夹模' in mold_list:
                ...
                self.Mold_Object['裁剪夹模_'] = DC0124_AD03_()
                # 这里的图号，是根据上面给的图号来的
            ...
            for mold_name in self.Mold_Object:
                ...
                # 获取新的图号，不同的模具有不同的图号，需要根据模具的名称来获取
                if mold_name == '成型退料模':
                    temp_number = self.get_numbers(mold_name, machine_type)
                    before = temp_number[:-4]
                    after = temp_number[-4:]
                    new_number = str(int(after) + 1)
                    if len(new_number) != 4:
